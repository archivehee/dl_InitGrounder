...
if world.dataset == 'Electronics':
    ...
    dataset = dataloader.InterCDRDataset(config=world.config, path=dataset_path)
elif world.dataset == 'lgn_cdr':
    dataset_path = os.path.join(_inter_cdr_root, "..", "lgn_cdr")
    dataset_path = os.path.abspath(dataset_path)
    dataset = dataloader.CrossDomainCDRDataset(config=world.config, path=dataset_path)
else:
